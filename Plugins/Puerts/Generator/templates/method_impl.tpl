    ...
    if (Info.Length() == {{overload.parameters | length}})
    {
            ...
            {%- endif %})
        {
            ...
            void* Ptr = new {{overload.return_type.variable_decl_type}}(MethodResult);
                {% if type.is_cdata %}
            V8Result = puerts::DataTransfer::FindOrAddCData(Isolate, Context, "{{overload.return_type.variable_decl_type}}", Ptr, false);
                {%- else %}
            V8Result = puerts::DataTransfer::FindOrAddStruct<{{overload.return_type.variable_decl_type}}>(Isolate, Context, Ptr, false);
                {% endif %}
            {%- elif overload.return_type.is_pointer -%}{#- 指针类型，和对象类型调用同样的接口-#}
                {%- if overload.return_type.is_pointee_type_primitive -%}
                    {% if type.is_cdata %}
            puerts::DataTransfer::FindOrAddCData(Isolate, Context, "{{overload.return_type.variable_decl_type}}", MethodResult, true);
                    {%- else %}
            puerts::DataTransfer::FindOrAddStruct<{{overload.return_type.variable_decl_type}}>(Isolate, Context, MethodResult, true);
                    {% endif %}
                {%- else -%}
                    {% if type.is_cdata %}
            puerts::DataTransfer::FindOrAddCData(Isolate, Context, "{{overload.return_type.pointee_type}}", MethodResult, true);
                    {%- else %}
            puerts::DataTransfer::FindOrAddStruct<{{overload.return_type.pointee_type}}>(Isolate, Context, MethodResult, true);
                    {% endif %}
                {%- endif -%}
            ...
            v8::String::NewFromUtf8(Isolate, TCHAR_TO_UTF8(*(MethodResult.ToString())), v8::NewStringType::kNormal).ToLocalChecked();
                {%- else -%}
            ...
            v8::{{overload.return_type.v8_type}}::New(Isolate, {%if overload.return_type.is_enum%}int32(MethodResult){%else%}MethodResult{%endif%});
            {%- endif %}
